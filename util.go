...
// example 1: result, err := ParseStr("first=foo&second=bar", result)
// returns 1: result => { first: "foo", second: "bar" }
// example 2: result, err := ParseStr("str_a=Jack+and+Jill+didn%27t+see+the+well.", result)
// returns 2: result => {"str_a":"Jack and Jill didn't see the well."}
// example 3: result := map[string]interface{}{"3": "a"}
// example 3: result, err := ParseStr(`a[b]["c"]=def&a[q]=t+5`, result)
// returns 3: result =>{"3":"a","a":{"b":{"c":"def"},"q":"t 5"}}
// example 4: result, err := ParseStr("a[][]=value", result)
// returns 4: result =>{"a":[["value"]]}
// example 5: result, err := ParseStr("a=1&a[]=2", result)
// returns 5: result =>{"a":[2]}
func ParseStr(str string, data ...map[string]interface{}) (interface{}, error) {
	if str == "" {
		return map[string]interface{}{}, nil
	}
	var build func(string, map[string]interface{}) error = func(str string, result map[string]interface{}) error {
		var rawurldecode func(string) (string, error) = func(str string) (string, error) {
			return url.QueryUnescape(strings.Replace(str, "%20", "+", -1))
		}
		str = regexp.MustCompile(`&$`).ReplaceAllString(regexp.MustCompile(`^&`).ReplaceAllString(str, ""), "")
		...
		for i := 0; i < len(strArr); i++ {
			tmp := strings.Split(strArr[i], "=")
			key, err = rawurldecode(tmp[0])
			if err != nil {
				return err
			}
			...
			value, err = rawurldecode(value)
			if err != nil {
				return err
			}
			if strings.Contains(key, "__proto__") || strings.Contains(key, "constructor") || strings.Contains(key, "prototype") {
				break
			}
			for key[0] == ' ' {
				key = key[1:]
			}
			if si := strings.Index(key, "\x00"); si > -1 {
				key = key[:si]
			}
			if key == "" || key[0] == '[' {
				continue
			...
			obj = result
			for j, k := range keys {
				key = regexp.MustCompile(`['"]$`).ReplaceAllString(regexp.MustCompile(`^['"]`).ReplaceAllString(k, ""), "")
				lastObj = obj
				if (key == "" || key == " ") && j != 0 {
					// Insert new dimension
					ct = -1
					for p, _ := range obj {
						pm := regexp.MustCompile(`^\d+$`).FindAllString(p, -1) //数字

						if pint := int(UInt(p)); pint > ct && len(pm) > 0 {
							ct = pint
						}

					}
					key = strconv.Itoa(ct + 1)
				}
				if obj[key] == nil {
					obj[key] = make(map[string]interface{})
				}
				if reflect.TypeOf(obj[key]).Kind() == reflect.Map {
	...
	//解析str到result
	err := build(str, result)
	if err != nil {
		return nil, err
	}
	var MapToSliceByKeys func(map[string]interface{}) interface{}
	MapToSliceByKeys = func(m map[string]interface{}) interface{} {
		var isKeyConseIntMap func([]string) bool = func(keys []string) bool {
			var dk int64 = -1 //起始下标
			for _, k := range keys {
		...
		if isKeyConseIntMap(keys) { //key 为 是连续的int的map
			var slice []interface{}
			for _, k := range keys {
				v := m[k]
				switch vt := v.(type) {
				case map[string]interface{}:
					v = MapToSliceByKeys(vt)
				}
	...
	return MapToSliceByKeys(result), nil
}

// recast - 尝试将字符串值转换为 bool 或 float64
...
func GetCharsetName(s string, isCode bool) string {
	...
	if isCode {
		resultCharset = regx.ExtractWithRegex2(s, `(?msi)<Meta[^>]*?resultCharset=["']?([-a-zA-Z_0-9]+)["']?`, regexp2.RegexOptions(options))
	} else {
		resultCharset = regx.ExtractWithRegex2(s, `(?msi)resultCharset=([-a-zA-Z_0-9]+)`, regexp2.RegexOptions(options))
	}
...
// StringListToIntList []string 转 []int
func StringListToIntList(slit []string) ([]int, error) {
	dist := make([]int, 0)
	for _, s := range slit {
...
// SubstrStartToLen 截取字符串 start 起点下标 length 需要截取的长度
func SubstrStartToLen(str string, start, length int) string {
	rs := []rune(str)
	rl := len(rs)
	end := 0
	if start < 0 {
...
// SubstrStartToEndIndex 截取字符串 start 起点下标 end 终点下标(不包括)
func SubstrStartToEndIndex(str string, start, end int) string {
	rs := []rune(str)
	length := len(rs)
...
// StringContainsIgnoreCase 字符串忽略大小写包含检测 源和目的字符串都忽略
func StringContainsIgnoreCase(s, substr string) bool {
	return strings.Contains(strings.ToLower(s), strings.ToLower(substr))
}

// StringIndexOfIgnoreCase 字符串查找 忽略大小写
func StringIndexOfIgnoreCase(s, substr string) int {
	return strings.Index(strings.ToLower(s), strings.ToLower(substr))
}

// String2IntWithDefaultValue 字符串转Int，如果转换失败，使用默认值
func String2IntWithDefaultValue(sourceStr string, defaultValue int) int {
	i, err := strconv.Atoi(sourceStr)
	if err != nil {
...
// String2Int64WithDefaultValue 字符串转Int64，如果转换失败，使用默认值
func String2Int64WithDefaultValue(sourceStr string, defaultValue int64) int64 {
	i, err := strconv.ParseInt(sourceStr, 10, 64)
	if err != nil {
		//fmt.Println(err.Error())
		return defaultValue
	}
...
// SplitStringToStringArray 分割字符串到字符串数组
func SplitStringToStringArray(s string, sep string) *[]string {
	strArray := strings.Split(s, sep)
	return &strArray
}

// SplitStringToIntArray 分割字符串到数字数组
func SplitStringToIntArray(s, sep string) *[]int {
	strArray := strings.Split(s, sep)
	intArray := make([]int, len(strArray))
	for _, v := range strArray {
		intArray = append(intArray, String2IntWithDefaultValue(v, 0))
	}
	return &intArray
}

// ObjContains 判断obj是否在target中，target支持的类型arrary,slice,map
func ObjContains(target interface{}, obj interface{}) (bool, error) {
...
// StringContainsArrayString 判断一个源字符串中是否包含一组字符串
func StringContainsArrayString(s *string, arr *[]string) bool {
	for _, v := range *arr {
...
// RegexMatchArrayRegexp 判断是否匹配一组正则表达式
func RegexMatchArrayRegexp(s *string, regexps *[]string) bool {
	for _, v := range *regexps {
		if !regx.RegexMatch(*s, v) {
			return false
		}
	}
	return true
...
// RemoveRepeatedElement 去除切片中的重复字符串
...
// FormatRawCookie 格式化字符串cookie为http.Cookie
func FormatRawCookie(raw string) []*http.Cookie {
	cookies := make([]*http.Cookie, 0)
	raw = textproto.TrimString(raw)
	var part string
	for len(raw) > 0 {
		if splitIndex := strings.Index(raw, ";"); splitIndex > 0 {
			part, raw = raw[:splitIndex], raw[splitIndex+1:]
		} else {
			part, raw = raw, ""
		}
...
func RandInt(min, max int) int {
	...
	z := big.NewInt(0)
	z.Sub(big.NewInt(int64(max)), big.NewInt(int64(min)))
	if z.Int64() > math.MaxInt32 {
		if min < 0 {
			if -min < 0 {
				return rand.Intn(max) - rand.Intn(math.MaxInt32)
			}
			return rand.Intn(max) - rand.Intn(-min)
		}
		return rand.Intn(math.MaxInt32 - min)
	}
	return min + rand.Intn(max-min)
}

// GenRandPhoneNumber 随机生成手机号
func GenRandPhoneNumber() string {
...
func GenRandIP() string {
	...
	binary.BigEndian.PutUint32(ip.To4(), rand.Uint32())
...
// RandLowerStr 随机英文小写字母
func RandLowerStr(len int) string {
	data := make([]byte, len)
	for i := 0; i < len; i++ {
...
// RandLowerNumberStr 随机英文小写字母加数字
func RandLowerNumberStr(len int) string {
	data := make([]byte, len)
	for i := 0; i < len; i++ {
		foo := rand.Intn(36)
		if foo < 26 {
			...
			data[i] = byte(foo - 26 + '0')
		}
	}
	return string(data)
}

// RandLUStr 随机英文大小写字符串
func RandLUStr(length int) string {
...
// RandUpperStr 随机英文大写字母
func RandUpperStr(len int) string {
	data := make([]byte, len)
	for i := 0; i < len; i++ {
...
// RandNumberLUStr 随机指定长度的混合字符串 (范围a-z,A-Z,0-9)
func RandNumberLUStr(length int) string {
	if length <= 1 {
		length = 2
	}
	digits := "0123456789"
...
// RandStrWithSpecial 生成随机包含大小写字母数字和特殊字符的字符串
func RandStrWithSpecial(length int) string {
	if length <= 1 {
		length = 2
	}
	digits := "0123456789"
...
// RandEmail 生成邮箱
func RandEmail(domainSuffix ...string) string {
	if len(domainSuffix) > 0 {
		return RandLowerStr(8) + "@" + domainSuffix[0]
	}
	return RandLowerStr(8) + "@" + RandLowerStr(5) + DomainSuffix[RandInt(0, len(DomainSuffix))]
}
...
func RandPassword(length int) string {
	...
	symbols := [...]string{"!", "@", "$", "*", "[", "]"}
	for i := int64(0); i < 2; i++ {
		str += string(rand.Int31n('Z'-'A') + 'A')
	}
	for i := int64(0); i < 2; i++ {
		str += string(rand.Int31n('9'-'0') + '0')
	}
	for i := int64(0); i < 1; i++ {
		str += symbols[rand.Intn(len(symbols))]
	}
	for i := int64(0); i < int64(length-5); i++ {
		str += string(rand.Int31n('z'-'a') + 'a')
	}
	return str
}
